...
* Reuse for any monad or nested monad stack (such as Future, Task, IO, writer, free or identity monad)
...
* Implement service code once and reuse for all monads and nested monads such as
Future, Task, IO, writer monad, free monad or identity monad (or any logical combination).
* Write monadic code using for-comprehensions on a complex monad stack
(such as Future[Writer[A]]) without the need for monad transformers
