...
* Declare an effectful service by creating a generic trait that accepts a generic monad parameter
* Any monad or nested monad stack (such as Future, Task, IO, writer, free, identity monad or any
  logical combination like Task[Writer[A]]) could be substituted.
* Trait contains only unimplemented methods (and maybe the occasional val)
...
** Note: which effects are captured depends on the monad selected and the implementation of this trait's
dependencies
...
==== Example: Implement the service generically
* Implement the service by creating a class that inherits from the trait and also has a generic monad
parameter.
* Constructor inject required service dependencies (hides dependencies from callers of trait).
* Implicitly inject Monad type-class and import monad operations to support for-comprehensions.
* Implicitly inject monad "augments" to ensure monad supports extra operations such as exception handling,
parallel execution or time delays.
* Keep service pure by deferring effect capture to other services or augments that capture the effects
inside the monad.
* Because monad is generic, map/flatMap and for-comprehensions can be used without the need for monad
transformers or lifting every statement.
